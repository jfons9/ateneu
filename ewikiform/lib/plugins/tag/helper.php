...
class helper_plugin_tag extends DokuWiki_Plugin {
    ...
    function tagLinks($tags) {
      ...
      $merge = array_merge($conf['tags_destinataris'],
         $conf['tags_tematiques'],
         $conf['tags_materies']);    
        ...
        foreach ($tags as $tag) {
          ...
          //  $tagsnou = array();
            foreach ($merge as $key => $value) {                                        
               // if (in_array(cleanID($key), $tags)){    
                if (array_key_exists($tag, $merge)){
    ...
    function getTopic($ns = '', $num = NULL, $tag = '') {
        ...
        foreach ($pages as $page) {
            ...
            // busquem  titol si està definit a ini.txt; si no, titol ocult html
            $fileini = $conf['savedir'].":pages:".$page.":ini.txt";
            $fileini = str_replace(':', '/', $fileini);
            $fileini = str_replace('/index', '', $fileini);
            if (file_exists($fileini)){
                $ini = $this->get_ini($fileini); 
            }
            ...
            if (isset($ini['titol'])){ 
                ...
                $titol = $this->titolcurs($page);
            }
            
            //$title = $titol ; //$meta['title'];
            $title = $titol;
  ...
  * 
  */
function titolcurs($page) {
    $fitxer = str_replace( ":", "/", $page);
    $codi = explode('/', $fitxer);
