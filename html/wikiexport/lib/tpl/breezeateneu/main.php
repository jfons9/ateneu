...
<html
    ...
    <body>
        <div class="dokuwiki">
           ...
           print('<ul class="seven columns" id="nav__menu">');
           print($menu);
           print('</ul>');
        ?>
...
?>
                ...
                <div class="nine columns" id="dokuwiki__content">
                    <?php
                     $helper = plugin_load('helper', 'ateneuplus');  
    $titol = $helper->get_titol($ID);  
    
    $fileini = $conf['savedir']."/pages/".trim($base)."/ini.txt";   
    $ini = $helper->get_ini($_SERVER["REQUEST_URI"]);     
    ...
    if ( isset($ini['mostratitol']) and trim($ini['mostratitol']) == "1" and isset($ini['titol']) and !empty($ini['titol'])){    
        ...
        if ($titol != 'nomenu' ){
           print('<div class="titol">'.$titol.'</div>');
        }           
    }
    
    $subtitol = $helper->get_subtitol($ID);  
    print('<div class="subtitol">'.$subtitol."</div>");
    ?>
                    <?php echo $buffer?>
