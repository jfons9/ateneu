...
class syntax_plugin_note extends DokuWiki_Syntax_Plugin {
    ...
    var $notes = array(
        ...
        'notecustom'    => array('custom', 'personalitzat')
      );
    ...
    function handle($match, $state, $pos, &$handler){
        global $conf; // per recuperar 'savedir'
        ...
        switch ($state) {
          ...
          case DOKU_LEXER_ENTER : 
            ...
            $fileini = $conf['savedir']."/pages/z_notecustom/notecustomdef.txt";
            $notecustom = $this->getNoteCustom($fileini) ;
              
            // afegit jordi  
            // Creem array separant $note  
            $params = preg_split('/\s+/', $note, 3); 
            // Assignem tipus de note
            $note = trim($params[0]);
                     
            foreach( $this->notes as $class => $names ) {
              if (in_array($note, $names)){
                return array($state, $class, trim($params[$note][1]), trim($params[$note][2]));
              }elseif(array_key_exists($note,$notecustom)){
                 return array($state, 'notecustom', trim($notecustom[$note][0]), trim($notecustom[$note][1]));
              }
            }            
    ...
    function render($mode, &$renderer, $indata) {
        ...
        if($mode == 'xhtml'){
          ...
          list($state, $data, $customcolor, $customicona) = $indata;
            ...
            case DOKU_LEXER_ENTER :             
               if ($data == "notecustom"){      
                  ...
                  if (!empty($customcolor)){
                      $renderer->doc .= ' style="background-color: '.$customcolor.';';
                      if (!empty($customicona)){    
                          $renderer->doc .= ' background-image: url(/wikiform/wikiform/lib/exe/fetch.php?media=:z_notecustom:'.$customicona.');';
                      }
                  }
                  $renderer->doc .= '">';
              }else{
     ...
     function getNoteCustom($file) {
        ...
        if (file_exists($file))   {
           ...
           foreach($linies as $linia){
              $trans = explode('|',$linia);
              $torna[trim($trans[1])] = array(trim($trans[2]),trim($trans[3]));
           }
        }else{
           return ;
        }      
